...
#########ElPaaso extend heroku script#########
VENDOR_BIN_DIR=$(cd $(dirname $0); pwd)
source $VENDOR_BIN_DIR/elpaaso
APP_DIR_ELPAASO=${__APP_NAME__}_APP_DIR
APP_DIR_ELPAASO=${!APP_DIR_ELPAASO}

# we need this in configs
HEROKU_APP_DIR="$APP_DIR_ELPAASO"
...
####ElPaaso cleaning####
unset HEROKU_APP_DIR 2>/dev/null
unset PORT 2>/dev/null
unset HEROKU_PHP_HTTPD_CONFIG_INCLUDE 2>/dev/null
########################